# rust-tiny-steps
🦀 300 tiny Rust exercises from zero to advanced - one concept at a time. Perfect for beginners who want to learn gradually.
# 🦀 300 Tiny Rust Exercises: Zero to Advanced
...
| **201–215: Basic Types** ||||
| 201 | Declare variables of different integer types: i8, u8, i16, etc. | Integer types | — | ☐ |
| 202 | Use integer literals with suffixes like 42u32 and handle overflow | Integer literals and overflow | — | ☐ |
| 203 | Use floating-point types f32 and f64, perform basic operations | Floating-point types | — | ☐ |
| 204 | Use boolean type, logical operations &&,, ! | Boolean type |  | ☐ |
| 205 | Declare char type, use Unicode characters | Character type | — | ☐ |
| 206 | Create a tuple and access its elements | Tuple basics | — | ☐ |
| 207 | Destructure a tuple into variables | Tuple destructuring | — | ☐ |
| 208 | Use the unit type () in a function | Unit type | — | ☐ |
| 209 | Create a type alias with type keyword | Type aliases | — | ☐ |
| 210 | Rely on type inference for variable declaration | Type inference | — | ☐ |
| 211 | Use the never type ! in a function that panics | Never type | — | ☐ |
| 212 | Print size of types using std::mem::size_of | Type sizes | — | ☐ |
| 213 | Cast between types using as operator | Type casting | — | ☐ |
| 214 | Convert between types using From and Into traits | Type conversions | — | ☐ |
| 215 | Use TryFrom and TryInto for fallible conversions | Fallible conversions | — | ☐ |
| **216–230: Modules and Crates** ||||
| 216 | Define a simple module with mod keyword | Module definition | — | ☐ |
| 217 | Make items public with pub keyword | Visibility | — | ☐ |
| 218 | Bring items into scope with use keyword | Use paths | — | ☐ |
| 219 | Re-export items with pub use | Re-exporting | — | ☐ |
| 220 | Use nested modules | Nested modules | — | ☐ |
| 221 | Split modules into separate files | Filesystem modules | — | ☐ |
| 222 | Create a library crate with Cargo | Crate creation | — | ☐ |
| 223 | Add an external crate dependency in Cargo.toml | Dependencies | — | ☐ |
| 224 | Use absolute and relative paths with :: and super | Path syntax | — | ☐ |
| 225 | Control struct field visibility | Struct visibility | — | ☐ |
| 226 | Use self in methods and paths | Self keyword | — | ☐ |
| 227 | Define items in the crate root | Crate root | — | ☐ |
| 228 | Install a crate from crates.io | Crates.io usage | — | ☐ |
| 229 | Use Cargo features in dependencies | Cargo features | — | ☐ |
| 230 | Create a binary and library in one crate | Dual crate types | — | ☐ |
| **231–245: Advanced Collections** ||||
| 231 | Create a HashMap and insert key-value pairs | HashMap basics | — | ☐ |
| 232 | Get values from HashMap with get | HashMap access | — | ☐ |
| 233 | Iterate over HashMap keys and values | HashMap iteration | — | ☐ |
| 234 | Use HashSet for unique elements | HashSet basics | — | ☐ |
| 235 | Create a BTreeMap for sorted keys | BTreeMap | — | ☐ |
| 236 | Use BTreeSet for sorted unique elements | BTreeSet | — | ☐ |
| 237 | Use VecDeque as a queue | VecDeque | — | ☐ |
| 238 | Use BinaryHeap as a priority queue | BinaryHeap | — | ☐ |
| 239 | Use HashMap entry API for or_insert | Entry API | — | ☐ |
| 240 | Implement custom hash for HashMap keys | Custom hashing | — | ☐ |
| 241 | Reserve capacity in Vec | Capacity management | — | ☐ |
| 242 | Use drain to remove elements from Vec | Drain iterator | — | ☐ |
| 243 | Extend a collection from an iterator | Extend trait | — | ☐ |
| 244 | Collect iterator into HashSet | Collect into sets | — | ☐ |
| 245 | Use linked_list for LinkedList | LinkedList | — | ☐ |
| **246–260: Advanced Generics** ||||
| 246 | Define a generic function over types | Generic functions | — | ☐ |
| 247 | Define a generic struct with type parameter | Generic structs | — | ☐ |
| 248 | Define a generic enum | Generic enums | — | ☐ |
| 249 | Define a generic trait | Generic traits | — | ☐ |
| 250 | Use const generics for fixed-size arrays | Const generics | — | ☐ |
| 251 | Define associated constants in traits | Associated consts | — | ☐ |
| 252 | Use turbofish syntax for generics | Turbofish | — | ☐ |
| 253 | Understand monomorphization | Monomorphization | — | ☐ |
| 254 | Combine generics and lifetimes | Generic lifetimes | — | ☐ |
| 255 | Add trait bounds to generics | Generic bounds | — | ☐ |
| 256 | Use newtype pattern for type safety | Newtype pattern | — | ☐ |
| 257 | Generic over array sizes with const | Array generics | — | ☐ |
| 258 | Impl blocks for generic types | Generic impls | — | ☐ |
| 259 | Default type parameters in generics | Default params | — | ☐ |
| 260 | Generic methods in impl blocks | Generic methods | — | ☐ |
| **261–275: Patterns and Matching Advanced** ||||
| 261 | Use match guards for extra conditions | Match guards | — | ☐ |
| 262 | Bind variables in match arms | Bindings in match | — | ☐ |
| 263 | Use ref in patterns for references | Ref patterns | — | ☐ |
| 264 | Match on slice patterns | Slice patterns | — | ☐ |
| 265 | Ignore parts with _ in patterns | Ignore patterns | — | ☐ |
| 266 | Use .. to ignore remaining tuple elements | Range ignore | — | ☐ |
| 267 | Use | for or patterns in match | Or patterns | — | ☐ |
| 268 | Use while let for pattern matching in loops | While let patterns | — | ☐ |
| 269 | Use let else for handling mismatches | Let else | — | ☐ |
| 270 | Use patterns in function parameters | Function param patterns | — | ☐ |
| 271 | Destructure structs in let bindings | Struct destructuring | — | ☐ |
| 272 | Destructure enums in match | Enum destructuring | — | ☐ |
| 273 | Use nested patterns in match | Nested patterns | — | ☐ |
| 274 | Understand match ergonomics | Match ergonomics | — | ☐ |
| 275 | Use if guards in match arms | If guards | — | ☐ |
| **276–290: Testing** ||||
| 276 | Write a simple unit test with assert! | Unit tests | — | ☐ |
| 277 | Use assert_eq! and assert_ne! | Equality assertions | — | ☐ |
| 278 | Test for panics with #[should_panic] | Panic testing | — | ☐ |
| 279 | Return Result from tests | Result in tests | — | ☐ |
| 280 | Write an integration test | Integration tests | — | ☐ |
| 281 | Write a doc test in comments | Doc tests | — | ☐ |
| 282 | Organize tests in modules | Test organization | — | ☐ |
| 283 | Use #[cfg(test)] for test-only code | Conditional compilation | — | ☐ |
| 284 | Ignore a test with #[ignore] | Ignoring tests | — | ☐ |
| 285 | Write a benchmark (unstable feature) | Benchmarking | — | ☐ |
| 286 | Test private functions | Private testing | — | ☐ |
| 287 | Follow test-driven development for a function | TDD example | — | ☐ |
| 288 | Use mock objects (with external crate) | Mocking | external | ☐ |
| 289 | Check test coverage (optional tool) | Test coverage | — | ☐ |
| 290 | Write tests for error cases | Error testing | — | ☐ |
| **291–300: I/O and Projects** ||||
| 291 | Read from a file using std::fs::read_to_string | File reading | — | ☐ |
| 292 | Write to a file using std::fs::write | File writing | — | ☐ |
| 293 | Use Path and PathBuf for file paths | Paths | — | ☐ |
| 294 | Handle I/O errors with Result | I/O error handling | — | ☐ |
| 295 | Read command-line arguments with std::env::args | CLI args | — | ☐ |
| 296 | Access environment variables | Env variables | — | ☐ |
| 297 | Serialize a struct to JSON with serde | Serialization | external | ☐ |
| 298 | Deserialize JSON to a struct with serde | Deserialization | external | ☐ |
| 299 | Spawn a child process with std::process::Command | Child processes | — | ☐ |
| 300 | **Mini Project**: Build a simple CLI todo app | Combining concepts | — | ☐ |
